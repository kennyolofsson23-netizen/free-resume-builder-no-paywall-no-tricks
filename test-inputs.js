...
async function test() {
  ...
  console.log(`  Placeholders: ${inputInfo.placeholders.filter(p => p).slice(0, 3).join(', ')}`);
  ...
  const structure = await page.evaluate(() => {
    ...
    return {
      headings: Array.from(headings).map(h => h.textContent.trim()).slice(0, 10),
  ...
  console.log(`  Main content starts with: ${structure.mainContent?.substring(0, 50)}...`);
