...
async function runTests() {
  ...
  try {
    ...
    const hasFreeMessage = await page
      .locator('text=/free|no paywall|no tricks/i')
      .count()
    console.log(`   ✓ Found ${hasFreeMessage} anti-paywall messages`)

    const buildBtn = page
      .locator('a[href="/builder"], button[onclick*="builder"]')
      .first()
    ...
    console.log(
      `   ${builderUrl.includes('/builder') ? '✓' : '✗'} Navigated to /builder`
    )

    // TEST 3: Form Rendering
    console.log('\n[3] Form Fields Rendering')
    const fullNameLabel = await page
      .locator('label:has-text("Full Name"), label:contains("Full Name")')
      .isVisible()
      .catch(() => false)
    const emailLabel = await page
      .locator('label:has-text("Email"), label:contains("Email")')
      .isVisible()
      .catch(() => false)
    const phoneLabel = await page
      .locator('label:has-text("Phone"), label:contains("Phone")')
      .isVisible()
      .catch(() => false)
    const locationLabel = await page
      .locator('label:has-text("Location"), label:contains("Location")')
      .isVisible()
      .catch(() => false)
    ...
    const templateBtns = await page
      .locator('button')
      .filter({ hasText: /Modern|Classic|Minimal/ })
      .all()
    ...
    for (const section of sections) {
      const isPresent = await page
        .locator(`heading:has-text("${section}"), h2:has-text("${section}")`)
        .isVisible()
        .catch(() => false)
