...
async function test() {
  ...
  try {
    ...
    if (addExpExists) {
      ...
      const companyInputs = await page.locator('input[placeholder*="Company"], input[placeholder*="company"]').count();
      console.log(`  ${companyInputs > 0 ? '✓' : '✗'} Experience form fields appeared`);
    }
    ...
    console.log(`  ${classicExists ? '✓' : '✗'} Classic template button visible`);
    ...
    const downloadBtn = page2.locator('button').filter({ hasText: /Download|PDF/ });
    ...
    if (downloadExists) {
      ...
      console.log(`  ${isDisabled ? '✓' : '✗'} Download button disabled until name is filled`);
    }
    ...
    if (shareExists) {
      ...
      await page.waitForTimeout(500);
      const hasModalOrDialog = await page.locator('dialog, [role="dialog"], [class*="modal"]').count() > 0;
      console.log(`  ${hasModalOrDialog ? '✓' : '✗'} Share dialog appears`);
    }

    // WORKFLOW 6: Dark mode toggle
    console.log('\n[WORKFLOW 6] Theme Toggle');
    const themeBtn = page.locator('button').filter({ hasText: /Moon|Sun|Dark|Light|Theme/i });
    const themeExists = await themeBtn.count() > 0;
    console.log(`  ${themeExists ? '✓' : '⚠'} Theme toggle button ${themeExists ? 'found' : 'not found (may not be implemented)'}`);
    ...
    console.log(`  ${hasUndo ? '✓' : '⚠'} Undo button ${hasUndo ? 'found' : 'not found'}`);
    console.log(`  ${hasRedo ? '✓' : '⚠'} Redo button ${hasRedo ? 'found' : 'not found'}`);
